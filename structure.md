...
- For Add and Edit presets actions, at the bottom of the action parameters panel, a message indicates when a parameter is linked to an existing field.
...
You can easily change the action sort criteria by selecting another field parameter using the dropdown list arrow. It will display the other field names available for a specific table.
...
## Action parameters linked to fields - iOS -

When linking your parameters to your fields on iOS, the MobileAppServer component makes your 4D code much easier in your On Mobile App Action database method. Indeed, the MobileAppServer component enables to speed up your 4D mobile app backend coding. Linking your fields to your action parameter will also allow you to use generic code and to map parameters and fields generically. Writing specific code will no longer be required! 

When creating and naming an action parameter linked to a field for a specific table, it shall have the exact same Name in the parameter properties panel.

However, for existing projects that already include an action parameter linked to a field with a name different to the field name:
- The existing action parameters (Name, Short and Long label) shall remain the same as long as the parameter's name value is not edited.
- The new action parameters that are created have the exact same Name in the parameter properties panel and in the field that it is linked with.

You are free  to link or unlink an action parameter by editing their name.
